...
public class EventCreateDtoValidator : AbstractValidator<EventCreateDto>
{
    public EventCreateDtoValidator()
    {
        ...
        RuleFor(x => x.Location).NotEmpty().WithMessage("地點不能為空");
        RuleFor(x => x.EventDate).GreaterThanOrEqualTo(DateTime.Today).WithMessage("活動日期不能是過去");
        RuleFor(x => x.TotalTickets).GreaterThan(0).WithMessage("總票數必須大於0");
